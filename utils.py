...
def preprocessing_rna_seq_data(rna_seq_data):
    ...
    for i, n in enumerate(names):
        ...
        if n == "CRY":
            rna_seq_matrix[i, :, :] = [
                list(np.nansum([l1, l2], axis=0))
                for l1, l2 in zip(
                    rna_seq_data["ctrl"]["CRY1"]["pts"],
                    rna_seq_data["ctrl"]["CRY2"]["pts"],
                )
            ]
        ...
        if n == "REV-ERB":
            rna_seq_matrix[i, :, :] = [
                list(np.nansum([l1, l2], axis=0))
                for l1, l2 in zip(
                    rna_seq_data["ctrl"]["NR1D1"]["pts"],
                    rna_seq_data["ctrl"]["NR1D2"]["pts"],
                )
            ]

        if n == "PER":
            rna_seq_matrix[i, :, :] = [
                list(np.nansum([l1, l2, l3], axis=0))
                for l1, l2, l3 in zip(
                    rna_seq_data["ctrl"]["PER1"]["pts"],
                    rna_seq_data["ctrl"]["PER2"]["pts"],
                    rna_seq_data["ctrl"]["PER3"]["pts"],
                )
            ]
        ...
        if n == "ROR":
            rna_seq_matrix[i, :, :] = [
                list(np.nansum([l1, l2], axis=0))
                for l1, l2 in zip(
                    rna_seq_data["ctrl"]["RORA"]["pts"],
                    rna_seq_data["ctrl"]["RORB"]["pts"],
                )
            ]

    col_arntl = np.concatenate(
        (rna_seq_matrix[0][:, 0], rna_seq_matrix[0][:, 1], rna_seq_matrix[0][:, 2]),
        axis=0,
    )
    col_clock = np.concatenate(
        (rna_seq_matrix[1][:, 0], rna_seq_matrix[1][:, 1], rna_seq_matrix[1][:, 2]),
        axis=0,
    )
    col_cry = np.concatenate(
        (rna_seq_matrix[2][:, 0], rna_seq_matrix[2][:, 1], rna_seq_matrix[2][:, 2]),
        axis=0,
    )
    col_rev = np.concatenate(
        (rna_seq_matrix[3][:, 0], rna_seq_matrix[3][:, 1], rna_seq_matrix[3][:, 2]),
        axis=0,
    )
    col_per = np.concatenate(
        (rna_seq_matrix[4][:, 0], rna_seq_matrix[4][:, 1], rna_seq_matrix[4][:, 2]),
        axis=0,
    )
    col_ror = np.concatenate(
        (rna_seq_matrix[5][:, 0], rna_seq_matrix[5][:, 1], rna_seq_matrix[5][:, 2]),
        axis=0,
    )

    rna_seq_matrix = pd.DataFrame(
        {
            "col1": col_arntl,
            "col2": col_clock,
            "col3": col_cry,
...
def new_preprocessing_rna_seq_data(rna_seq_data, factor=1):
    paralogs = [
        "ARNTL",
        "CLOCK",
        "CRY1",
    ...
    for i in range(len(names)):
        ...
        nansum = (
            np.nansum(concat, axis=0) / factor
        )  # multiply this by 10**3 for pmol/L visualization
        gene, ct = [], []
