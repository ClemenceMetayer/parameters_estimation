...
from functions import from_log_010_to_ab, wrapper_fit, fitness, clock_model
from utils import preprocessing_rna_seq_data, preprocessing_prot_data, get_parameter_bounds_percent, get_random_parameters, bounds_reach
...
def main():
    ...
    nanmean = np.nanmean(rna_seq_matrix, axis=0)
    inds = np.where(np.isnan(rna_seq_matrix))
    rna_seq_matrix[inds] = np.take(nanmean, inds[1])
    
    # Time points initialisation 
    ...
    jeu2_matrix, jeu2_timepoints, jeu3_matrix, jeu3_timepoints, jeu4_matrix, jeu4_timepoints, jeu5_matrix, jeu5_timepoints, jeu7_matrix, jeu7_timepoints, jeu8_matrix, jeu8_timepoints, jeu10_matrix, jeu10_timepoints = preprocessing_prot_data(prot_data)
    ...
    with EvalParallel2(wrapper_fit) as eval_all:
        for _ in range(10):
            ...
            if es.result[1] < likelihood:
                ...
                np.savetxt(
                    f"results/example_new _cmaes_result_{likelihood}.txt",
